 ...
 */
//DustSensor sensor(P1_UART_TX, P1_UART_RX, BAUD_RATE);
...
static void send_message()
{
    ...
    hpma115_data_t data;
    //requestDataAutosend(buffer_read, MAX_BUFFER_SIZE);
    ...
    //size_t len = sprintf((char*)tx_buffer, "{\"temperature\": 21.01}" );
    size_t len1 = sprintf((char*)tx_buffer, "{\"Pm 2.5\":%d,\"Pm 10\":%d, \"Pm 1;0\":%d, \"Pm 4;0\":%d }", data.pm2_5, data.pm10, data.pm1_0, data.pm4_0);

    retcode = lorawan.send(MBED_CONF_LORA_APP_PORT, tx_buffer, len1,
                           MSG_UNCONFIRMED_FLAG);
    if (retcode < 0) {
